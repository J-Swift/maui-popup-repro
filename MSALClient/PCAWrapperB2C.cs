...
namespace MauiB2C.MSALClient
{
    ...
    public class PCAWrapperB2C
    {
        ...
        /// <param name="scopes">desired scopes</param>
        /// <returns></returns>
        public async Task<bool> AcquireTokenInteractiveAsync(UIViewController parent)
        {
            var accounts = await PCA.GetAccountsAsync(B2CConstants.PolicySignUpSignIn).ConfigureAwait(false);
            var account = accounts.FirstOrDefault();

            try
            {
                if (accounts == null || !accounts.Any())
                    throw new MsalUiRequiredException("31", "Need UI");

                var authResult = await PCA.AcquireTokenSilent(B2CConstants.Scopes, account).ExecuteAsync().ConfigureAwait(false);

                //await Microsoft.Maui.Storage.SecureStorage.SetAsync(Constants.AccessToken, authResult?.IdToken);
            ...
            catch (MsalUiRequiredException)
            {
                try
                {
                    var authResult = await PCA.AcquireTokenInteractive(B2CConstants.Scopes)
                        .WithParentActivityOrWindow(parent)
                        .WithUseEmbeddedWebView(false)
                        .ExecuteAsync()
                        .ConfigureAwait(false);

                    //await SecureStorage.SetAsync(Constants.AccessToken, authResult?.IdToken);
                ...
                {
                    Console.WriteLine("Authn Error1");
            ...
            {
                Console.WriteLine("Authn Error2");
                Console.WriteLine(e.Message);
                // SaveToken(null);
                return false;
            }
